export function buildPhasePrompt(phase, { issue, plan = "", validation = "", diff = "", trustedSkills = null }) {
  ...
  if (phase === "implement") {
    ...
    return `${boundary}\nImplement the approved plan as one focused behavioral change. The planning phase already performed broad research. Do not repeat exhaustive repository or skill discovery; inspect only the files needed for the approved plan and begin concrete edits early. Write behavior tests where practical. Do not merely describe the solution. Planner session-status text is not part of the plan and must never postpone implementation. For QA- or evidence-focused issues, implement an executable automated QA harness plus its evidence-writing logic and add a package script named qa; use exactly @playwright/test 1.62.0 when real browser behavior is required. The scripts test, typecheck, lint, format:check, and qa are all mandatory for QA work. Do not fabricate observed results in static prose. The later secret-free isolated workflow stage executes qa with Chromium and records the real evidence.\n\n--- APPROVED PLAN ---\n${actionablePlan}\n--- END APPROVED PLAN ---${context}\n\n--- EXECUTION DIRECTIVE ---\nCommand execution is intentionally unavailable in this implementation phase. Validation commands mentioned in the plan belong to the workflow stage that runs after you finish editing. Do not run validation commands and do not postpone or refuse implementation because commands cannot run here. Start writing the required code and tests now; for QA issues this includes the QA harness and evidence generator. The workflow will validate them next.\n--- END EXECUTION DIRECTIVE ---`;
  }
  if (phase === "repair") {
    const actionablePlan = stripPlannerStatus(plan);
