...
const merged = await github(`/repos/${event.repository.full_name}/pulls/${pull.number}/merge`, { method: "PUT", body: { merge_method: "squash" } });
if (!merged.merged) throw new Error(`GitHub did not merge PR #${pull.number}: ${merged.message || "unknown"}`);
